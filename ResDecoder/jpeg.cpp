...
BMP *Unpak_jpg(unsigned char *inbuf, unsigned int insize)
{
	...
	if (*(WORD *)jpeg_buf != 0xd8ff)
	{
		// 不是 JPEG 文件
		return NULL;
	}
	jpeg_buf = (void *)((WORD *)jpeg_buf + 1);

	init_table();

	// 读文件头
	do {
		while (*(BYTE *)jpeg_buf != 0xff)
			jpeg_buf = (void *)((BYTE *)jpeg_buf + 1);
		jpeg_buf = (void *)((BYTE *)jpeg_buf + 1);
		while ((type = *(BYTE *)jpeg_buf) == 0xff)
			jpeg_buf = (void *)((BYTE *)jpeg_buf + 1);
		jpeg_buf = (void *)((BYTE *)jpeg_buf + 1);
		switch (type) {
		case 0x01: break;
...
BMP *Unpak_mapx(unsigned char *inbuf, unsigned int insize)
{
	...
	HDU = *(unsigned short *)jpeg_stream;
	jpeg_stream += 2;
	VDU = *(unsigned short *)jpeg_stream;
	jpeg_stream += 2;
	...
	if (jpeg_DRI>0) {
		...
		case 0: // 灰度 Jpeg
			for (i = 0; i<VDU; i += 8) {
				jpeg_stream += ((unsigned short)jpeg_bit + 7) / 8;
				jpeg_DC[0] = *(unsigned short *)jpeg_stream;
				jpeg_stream += 2;
				jpeg_DC[1] = *(unsigned short *)jpeg_stream;
				jpeg_stream += 2;
				jpeg_DC[2] = *(unsigned short *)jpeg_stream;
				jpeg_stream += 2;
